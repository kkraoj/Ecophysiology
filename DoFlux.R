...
#  Calculate Coupled photosynthesis - energy budget - conductance system
# ----------------------------------------------------------
# Inputs  ca (ppm *1e-06), rh (%), ta (oC), SWd (W m-2), gb (mol m-2 s-1),
#  rstfac (stress factor), ty (veg type)
# ----------------------------------------------------------
...
#  VP_K.R = saturation vapor pressure (Pa) at t (oC)
#  dVP_K.R= d(svp)/dt
#  doPS_pCO2c.R
#  doPS_g0.R
#  doEG_g1.R
...
# Required package (install it if you haven't already):
# library(polynom)
 library(pracma)
...
#you don't need the semicolons in R 
swdown=587
zlwd=466.5
em=28.3
tm=304.1
...
ty=1
# change value of ty to change biomes
#names(Ps_params)=c('c4','vmax_umol','smax','zkc_p','zko_p','rrkk_p',
#	'leaf_Tc','apar_umol','Ca_ppm','ptot','pO2m','PARalb','emis',
#	'nitratefac','rstfac1','effcon','btheta','hhti','hlti','respcp',
#	'atheta','shti','slti','trda','trdm','gradm','bintc')
            
#global  Ps_params   
    c4=Ps_params[ty,'c4']   		#Photosynthetic type c3=0 c4=1
    vmax_umol=Ps_params[ty,'vmax_umol'] * cif   #Rubisco, NOTE: mol m-2 s-1 
    smax=Ps_params[ty,'smax']        #C3 POS factor (unused in C4)
    ...
    leaf_Tc =Ps_params[ty,7]    # default Input leaf temperature,oK
...
#  ea = water vapor pressure of air
#ea = VP_K(tka) * rh / 100

#calculate down-welling longwave radiation (Brutsaert equation ea in kPa)

 ema=1.24*((ea /100)/tka)^(1/7) #(note: must convert ea (Pa) to ea (mbar)
# LWd=ema * sbc *(tka)^4
# ema2=0.66+(0.039 * ea /100)^0.5#Brunt's equation
# LWD2=ema2 *sbc *(tka)^4
